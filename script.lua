...
local function RoundVector3(Vector)
 return Vector3.new(math.floor(Vector.X), math.floor(Vector.Y),
math.floor(Vector.Z))
end
...
local Ragdoll =
ReplicatedStorage:WaitForChild("Events"):WaitForChild("Player"):WaitForChild("Ragdo
ll")
local DebrisFolder = workspace:WaitForChild("Debris")
local HitChance = 100
local Repository =
"https://raw.githubusercontent.com/mstudio45/LinoriaLib/refs/heads/main/"
local Library = loadstring(game:HttpGet(Repository .. "Library.lua"))()
local ThemeManager = loadstring(game:HttpGet(Repository ..
...
local VelocityFly =
loadstring(game:HttpGet("https://raw.githubusercontent.com/centerepic/VelocityFly/
main/VelocityFly.lua"))()
local Aiming =
loadstring(game:HttpGet("https://raw.githubusercontent.com/centerepic/
sasware_blackout/main/aiming_lib.lua"))()
...
local function GetData(Player : Player, GetCharacterData : boolean)
 ...
 if GetCharacterData and Player:GetAttribute("LoadedCharacterData") == true then
-- If the player has spawned in from the menu
...
local function GetWithinRange(Instances : {Model | BasePart}, Position : Vector3,
Range : number)
 local Objects = {}
 for _, Object in ipairs(Instances) do
 local InstancePosition = Object:GetPivot().Position
...
function BypassTP(Target: Vector3 | CFrame | PVInstance): RBXScriptSignal
 ...
 local VecticalDist = math.abs(Start.Position.Y + 22) +
math.abs(Pivot.Position.Y + 22)
 ...
 local localAlpha = (Alpha - DropFraction - MiddleFraction) /
RiseFraction
...
Functions = {
 ...
 Library:Notify("Auto-Revive | You have no faction, cannot autorevive.")
 end
 if Faction ~= nil then
 PreTeleport()
 ...
 local Closest, _ = GetClosest(FriendlyNPCs,
...
Functions.GotoNearestMerchant = function()
 ...
 local Closest, _ = GetClosest(Merchants,
LocalPlayer.Character:GetPivot().Position)
 return BypassTP(Closest:GetPivot() + Closest.PrimaryPart.CFrame.LookVector.Unit
* 3), Closest
end
Functions.GotoNearestBroker = function()
 ...
 local Closest, _ = GetClosest(Brokers,
LocalPlayer.Character:GetPivot().Position)
 return BypassTP(Closest:GetPivot() + Closest.PrimaryPart.CFrame.LookVector.Unit
* 3), Closest
end
Functions.GotoNearestTerminal = function()
 PreTeleport()
 local Closest, _ = GetClosest(Terminals:GetChildren(),
LocalPlayer.Character:GetPivot().Position)
...
Functions.FilterZeroMoneyObjects = function(Loot : {Model})
 local Filtered = {}
 for i, Item in next, Loot do
 if Attribute(Item.LootBase.LootTable, "Cash") == 0 and
Attribute(Item.LootBase.LootTable, "Valuables") == 0 then
...
Functions.GetClosestLootObjectOfType = function(Position : Vector3, Types :
{string}?, FilterMoney : boolean?)
...
Collect(ProximityPromptService.PromptButtonHoldBegan:Connect(function(ProximityProm
pt : ProximityPrompt)
...
local FloorHideCoroutine = coroutine.create(function()
 ...
 CFrame.new(OriginalPivot.Position - (Vector3.yAxis * 6),
OriginalPivot.Position)
 )
 Functions.BreakVelocity()
 end
 else
...
local AutofarmCoroutine = coroutine.create(function()
 ...
 local Target =
Functions.GetClosestLootObjectOfType(LocalPlayer.Character:GetPivot().Position,
FarmTypes, true)
 if Target then
 ...
 if
Target.Lid.Lockpick:FindFirstChild("LockMinigame") then
...
fireproximityprompt(Target.Lid.Lockpick.LockMinigame)
 ...
 UnlockWaitConnection =
Remotes.Minigame.OnClientEvent:Once(function()
 ...
 Remotes.LootObject:FireServer(LootTable, "Valuables")
 Remotes.LootObject:FireServer(LootTable, "Cash")
 until Attribute(LootTable, "Cash") == 0 and Attribute(LootTable,
"Valuables") == 0
...
local KillAuraCoroutine = coroutine.create(function()
 ...
 local Closest, ClosestRange = GetClosest(FilteredPlayers,
LocalPlayer.Character:GetPivot().Position)
 ...
 if NPC:IsDescendantOf(workspace) and NPC:IsA("Model") and
NPC:FindFirstChild("HumanoidRootPart") and (CollectionService:HasTag(NPC,
"ActiveCharacter") or game.PlaceId == Nightbound) then
 table.insert(FilteredNPCs, NPC)
 end
 end
 if game.PlaceId ~= Nightbound then
 ...
 if Child:FindFirstChildOfClass("Humanoid") and not
Child:GetAttribute("Downed") then
 table.insert(FilteredNPCs, Child)
 end
 end
 end
 local Closest, ClosestRange = GetClosest(FilteredNPCs,
LocalPlayer.Character:GetPivot().Position)
 if Closest and ClosestRange <= Range then
...
do
 ...
 task.wait(0.1)
 Remotes.TransferCurrency:FireServer("Deposit",
Attribute(LocalPlayer.PlayerGui, "Cash"))
 return BypassTP(Origin):Wait()
 end)
end
do
...
do
 local VisualModificationsGroupBox =
Tabs.Visuals:AddLeftGroupbox('Modifications')
 ...
 LocalPlayer.PlayerGui.MainGui.Minimap.TabsFrame.MapFrame.Visible =
true
 else
 if LocalPlayer.PlayerGui.MainGui.Minimap.TabsFrame.Edge.ImageColor3
~= Color3.fromRGB(101, 255, 84) then
...
function(self, ...)
 ...
 if Toggles.NoFall.Value or Toggles.Flight.Value or
shared.State.NoFall then
 if Args[1] ~= 1000 then -- Resetting does 1000 damage, so
we ignore that
 return
 end
 end
 elseif self.Name == "Ragdoll" then
 if Toggles.NoRagdoll.Value or Toggles.Flight.Value or
shared.State.NoRagdoll then
 return
 end
...
Aiming.CurrentTarget[Options.SilentAimParts.Value],
 ...
 if AttributeSpoof[self] and AttributeSpoof[self].Key == Args[1]
then
...
FireHook = function(...)
 ...
 if Aiming.CurrentTarget then
 Args[3] =
(Aiming.CurrentTarget[Options.SilentAimParts.Value].Position - Args[2])
 end
 end
 end
...
'RightControl', NoUI = true, Text = 'Menu keybind' })
